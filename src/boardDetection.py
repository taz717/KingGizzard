###############################################################################
# This file will be used to create the board object that will be used to
# determine the state of the board. This just means it will detect the player
# move for the purposes of this project
###############################################################################
...
class Board_Img:
    ...
    def get_space(self, grid):
        ...
        for keys in keyList[:-1]:
            ...
            for i in range(0, (len(grid["0"]) - 1)):
                space = Space(
                    grid[keys][i], grid[keys][i + 1], grid[str(int(keys) + 1)][i]
                )
    ...
    def in_border(self, x, y):
        if (
            self.border[0] <= x <= self.border[1]
            and self.border[2] <= y <= self.border[1]
        ):
            return True
        else:
            return False
    ...
    def player_move(self, centeriod, moves):
        ...
        # using regex we can determine the moves that are potential caputures
            ...
            moves = moves[3:]
            takes = [
                re.findall(r"x([a-h][1-8])", move)[0]
                for move in moves
                if re.findall(r"x[a-h][1-8]", move)
            ]
            for i in takes:
...
def draw_points(grid):
    img = cv2.imread(
        "C:\\Users\Gordon\\OneDrive\\Desktop\\School\\cmpt496\\KingGizzard\\images\\chessboard.png"
    )
